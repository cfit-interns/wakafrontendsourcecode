...
import {Bookings, BookingsPagesProps} from "../models/bookings";
import {Rosters} from "../models/rosters";
import {Calendars} from "../models/calendars";


export async function getDestinationSuggestions(date:string): Promise<Rosters[]> {
  const response = await fetchData(`/api/bookings/suggestBooking?date=${date}`, {method: "GET"});
...
export async function resetPassword(credentials: ResetPasswordCredentials,id:string) {
  const response = await fetchData(`/api/users/userresetpassword/${id}`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
export async function userForgotPassword(credentials: UserForgotPassowrdCredentials): Promise<User> {
  const response = await fetchData("/api/users/userforgotpassword",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
export async function staffRegister(credentials: StaffLoginCredentials): Promise<Staff> {
  const response = await fetchData("/api/staffs/staffsignup",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
export interface AllBookingsCredentials {
  name: string,
  limit: number,
  page: number,
  date: string,
...
export async function fetchAllBookings(credentials: AllBookingsCredentials): Promise<BookingsPagesProps> {
  const response = await fetchData("/api/bookings/staffview", {
    method: "POST", headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify(credentials),
...
export interface dateArrProps {
  dateArr: string[],
}

export async function getMonthCalendars(dateArr: dateArrProps): Promise<any> {
  const response = await fetchData("/api/calendars/getMonthCalendars",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
