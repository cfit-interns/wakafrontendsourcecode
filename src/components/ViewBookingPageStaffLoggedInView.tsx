...
const ViewBookingPageStaffLoggedInView = () => {
    ...
    useEffect(() => {
        async function loadBookings() {
            try {
                ...
                const bookings = await WebsitesApi.fetchAllBookings({page: 1, limit: 5, name: '',date: ''});
        ...
        <>
            ...
            {!bookingLoading && !showBookingLoadingError &&
                <>
                    {bookings.length > 0
                        ? bookingsGrid
                        : <p>There are no currently booking requests</p>
                    }
                </>
            }
        </>
...
export default ViewBookingPageStaffLoggedInView;
